 ...
 * <ul>
 * <li> `browser:uncompressed` - build uncomprssed browser bundle (`viacore-*.js`)
 * <li> `browser:compressed` - build compressed browser bundle (`viacore-*.min.js`)
...
function startGulp(name, opts) {
  ...
  var fullname = name ? 'viacore-' + name : 'viacore';
  var files = ['lib/**/*.js'];
  var tests = ['test/**/*.js'];
  var alljs = files.concat(tests);

  var buildPath = './node_modules/viacore-build/';
  ...
  if (browser) {
    ...
    if (name !== 'lib') {
      browserifyCommand = browserifyPath + ' --require ./index.js:' + fullname + ' --external viacore-lib -o ' + fullname + '.js';
    } else {
      browserifyCommand = browserifyPath + ' --require ./index.js:viacore-lib -o viacore-lib.js';
    }
